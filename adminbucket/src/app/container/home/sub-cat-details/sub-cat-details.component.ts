...
export class SubCatDetailsComponent implements OnInit {
  ...
  gettingData() {
    this.loader = true;
    const url = this.base_path_service.base_path_api + 'subCategory/particularSubCategory/' + this.id;
    this.base_path_service.GetRequestUnauthorised(url)
      .subscribe(res => {
        this.loader = false;
  ...
  saveData() {
    this.loader = true;
    const url = this.base_path_service.base_path_api + 'subCategory/updateSubCategory/' + this.id;
