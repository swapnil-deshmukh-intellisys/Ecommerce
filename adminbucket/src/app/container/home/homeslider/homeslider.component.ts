...
export class HomesliderComponent implements OnInit {
  ...
  fileChangeEvent(event) {
    ...
    if (this.image) {
      ...
      if (this.image.size <= 5242880 && filename == 'jpeg' || this.image.size <= 5242880 && filename == 'png' || this.image.size <= 5242880 && filename == 'jpg' || this.image.size <= 5242880 && filename == 'svg') {
        const url = this.httpservice.base_path_api + 'user/uploadImage';
  ...
  gettingCategory() {
    const url = this.httpservice.base_path_api + 'category/categoryDropdown';
  ...
  CategoryChange(event) {
    const url = this.httpservice.base_path_api + 'subCategory/allSubCategory/' + event.value;
    this.httpservice.GetRequestUnauthorised(url)
      .subscribe(res => {
  ...
  saveData() {
    this.loader = true;
    const url = this.httpservice.base_path_api + 'offer';
